...
def get_minibatch_with_class(input_seqs, tag_seqs, class_labels, word2idx, tag2idx, class2idx, train_data_indx, index,
                             batch_size, add_start_end=False, multiClass=False, enc_dec_focus=False, device=None):
    """Prepare minibatch."""
...
def decode(seed_data, device, model_path, vocab_path, save_npy, states_path, test_batchSize=1):
    ...
    test_feats, test_tags, test_class = \
        read_seqtag_data_with_class(seed_data, word_to_idx, tag_to_idx, class_to_idx, multiClass=False, lowercase=False)
    ...
    # optimizer = optim.Adam(params, lr=0.001, betas=(0.9, 0.999), eps=1e-8, weight_decay=0)  # (beta1, beta2)

    model_tag.eval()
    model_class.eval()

    data_index = np.arange(len(test_feats))
    losses = []
    TP, FP, FN, TN = 0.0, 0.0, 0.0, 0.0
    TP2, FP2, FN2, TN2 = 0.0, 0.0, 0.0, 0.0
    # TODO: when seed_data is larger than 500, should use minibatch and combaine the output npy


    inputs, tags, raw_tags, classes, raw_classes, lens = get_minibatch_with_class(
        input_seqs=test_feats['data'], tag_seqs=test_tags['data'], class_labels=test_class['data'], word2idx=word_to_idx,
        tag2idx=tag_to_idx, class2idx=class_to_idx, train_data_indx=data_index, index=0,
        batch_size=test_batchSize, add_start_end=False, multiClass=False, enc_dec_focus=False, device=device)
    # tag_scores, encoder_info = model_tag(inputs, lens, with_snt_classifier=True)
    tag_scores, (packed_h_t_c_t, lstm_out, lengths) = \
                    model_tag(inputs, lens, with_snt_classifier=True)
    ...
    if save_npy:
        ...
        # np.save("../save_profile/snips/out/out_" + "0_batch_0" + ".npy", lstm_out.cpu().detach().numpy())
    ...
    return slot_accuracy, intent_accuracy, mean_losses, p, r, f, 0 if 2 * TP2 + FN2 + FP2 == 0 \
        else 100 * 2 * TP2 / (2 * TP2 + FN2 + FP2)


def eval_model(seed_data, states_path):
    ...
    model_path = "/home/cici/major/DialTest/save_profile/snips/model"
    vocab_path = "/home/cici/major/DialTest/save_profile/snips/vocab"

    start_time = time.time()
    acc_slot_te, acc_intent_te, loss_te, p_te, r_te, f_te, cf_te = \
        decode(seed_data=seed_data, device=device, model_path=model_path, vocab_path=vocab_path, save_npy=True,
               states_path=states_path, test_batchSize=1)
